...
export default function Live() {
  axios
    .get("http://localhost:5000/api/live/testa", { withCredentials: true })
    .then((response) => {});
  const [UserInLive, setUserInLive] = useState([]);
  useEffect(() => {
    axios.get("http://localhost:8090/api/streams").then((response) => {
      if (response.status) {
...
export function UserLink() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8090/api/streams")
      .then((response) => {
        if (response.status) {
          if (Object.keys(response.data).length > 0) {
  ...
  return (
    <>
      {Test.length == 0 && <p>Personne n'est en direct pour le moment</p>}
