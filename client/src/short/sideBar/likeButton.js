...
class LikeButton extends React.Component{
  ...
  like() {
    if (!this.props.isLiked) {
      this.props.setState((state) => ({ likes: state.likes + 1, isLiked: true }));
    ...
    render(){
      return(
        <div className='flex flex-col items-center'>
          {/* Like button */}
          <button id="like" className='h-[5vh] w-[5vh] flex items-center justify-center rounded-full bg-[#f5f5f5] hover:bg-[#e5e5e5]' onClick={this.like}>
            <img src="like.png" id="likeImg" className='scale-50' />
          </button>
          {/* Likes count */}
          <p className='text-[1.5vh]'> {/*this.props.videosInfos['nb_likes']*/ this.props.likes}</p>
        </div>
      )
    }
}
