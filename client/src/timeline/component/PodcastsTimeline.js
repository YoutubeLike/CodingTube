    ...
    indents.push(
      <div>
        <p className="p-5 bg-red-700 text-white rounded-lg">
          No podcast video in BDD... Publish a podcast video to become the
          first!
        </p>
      </div>
    );
  ...
  for (var i = 0; i < videosInfos.length; i++) {
    ...
    indents.push(
      <div key={i} className="mb-10 flex content-center">
        <a href={"http://localhost:5000/api/channel/thumbnail?idThumbnail=" + videosInfos[i]["id"]}>
          <div class="sm:block md:flex md:flex-row">
            <div class="relative">
              <img
                className="md:max-w-[300px] sm:max-w-auto h-auto rounded-lg"
                src={`/video?id=${videosInfos[i]["id"]}`}
                alt="Thumbnail"
              />
